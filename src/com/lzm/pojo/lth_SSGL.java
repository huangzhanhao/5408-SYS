package com.lzm.pojo;

public class lth_SSGL {

	private String changdi;
	private String shenqingren;
	private String kaishishijian;
	private String jieshushijian;
	private String shenqingriqi;
	private String zhifubiaoji;
	private String dingdanjine;
	private String qicai;
	
	private String QCname;
	private String QCmoney;
	private String QCsign;
	private String CDname;
	private String CDmoney;
	private String CDsign;
	private String CD;	
	private String QC;
	private String shenhebiaoji;
	
	private String yonghucaozuo;
	
	private String CDid;
	
	private String QCid;
	
	
	
	public String getYonghucaozuo() {
		return yonghucaozuo;
	}
	public void setYonghucaozuo(String yonghucaozuo) {
		this.yonghucaozuo = yonghucaozuo;
	}
	
	
	public String getShenhebiaoji() {
		return shenhebiaoji;
	}
	public void setShenhebiaoji(String shenhebiaoji) {
		this.shenhebiaoji = shenhebiaoji;
	}
	
	
	
	public String getQCid() {
		return QCid;
	}
	public void setQCid(String QCid) {
		this.QCid = QCid;
	}
	
	
	
	public String getCDid() {
		return CDid;
	}
	public void setCDid(String CDid) {
		this.CDid = CDid;
	}
	
	
	
	
	
	
	public String getCD() {
		return CD;
	}
	public void setCD(String CD) {
		this.CD = CD;}
		
		
	
	public String getQC() {
		return QC;
	}
	public void setQC(String QC) {
		this.QC = QC;}
		
	
	
	
	
	
	
	
	public String getQCname() {
		return QCname;
	}
	public void setQCname(String QCname) {
		this.QCname = QCname;
		
		
		
	}	public String getQCmoney() {
		return QCmoney;
	}
	public void setQCmoney(String QCmoney) {
		this.QCmoney = QCmoney;
		
		
		
	}	public String getQCsign() {
		return QCsign;
	}
	public void setQCsign(String QCsign) {
		this.QCsign = QCsign;
		
		
		
	}	public String getCDname() {
		return CDname;
	}
	public void setCDname(String CDname) {
		this.CDname = CDname;
		
		
		
	}	public String getCDmoney() {
		return CDmoney;
	}
   public void setCDmoney(String CDmoney) {
		this.CDmoney = CDmoney;
		
		
		
	}	public String getCDsign() {
		return CDsign;
	}
	
	public void setCDsign(String CDsign) {
		this.CDsign =CDsign;
	}
	
	
	
	
	public String getQicai() {
		return qicai;
	}
	public void setQicai(String qicai) {
		this.qicai = qicai;
	}
	
	public String getChangdi() {
		return changdi;
	}
	public void setChangdi(String changdi) {
		this.changdi = changdi;
	}
	public String getShenqingren() {
		return shenqingren;
	}
	public void setShenqingren(String shenqingren) {
		this.shenqingren = shenqingren;
	}
	public String getKaishishijian() {
		return kaishishijian;
	}
	public void setKaishishijian(String kaishishijian) {
		this.kaishishijian = kaishishijian;
	}
	public String getJieshushijian() {
		return jieshushijian;
	}
	public void setJieshushijian(String jieshushijian) {
		this.jieshushijian = jieshushijian;
	}
	public String getShenqingriqi() {
		return shenqingriqi;
	}
	public void setShenqingriqi(String shenqingriqi) {
		this.shenqingriqi = shenqingriqi;
	}
	public String getZhifubiaoji() {
		return zhifubiaoji;
	}
	public void setZhifubiaoji(String zhifubiaoji) {
		this.zhifubiaoji = zhifubiaoji;
	}
	public String getDingdanjine() {
		return dingdanjine;
	}
	public void setDingdanjine(String dingdanjine) {
		this.dingdanjine = dingdanjine;
	}
	@Override
	public String toString() {
		// TODO Auto-generated method stub
		return super.toString();
	}
}

